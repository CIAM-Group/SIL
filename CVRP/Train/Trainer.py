    ...
    def repair_one_batch(self, episode, batch_size, problem_size_type, clock=None):
        ...
        with torch.no_grad():
            ...
            for bbbb in range(budget):
                ...
                for i in range(2):
                    best_select_node_list[int(batch_size / 2) * i:int(batch_size / 2) * (i + 1)] = \
                        self.env.Rearrange_solution_clockwise(
                            self.origin_problem[int(batch_size / 2) * i:int(batch_size / 2) * (i + 1)],
                            best_select_node_list[int(batch_size / 2) * i:int(batch_size / 2) * (i + 1)])
